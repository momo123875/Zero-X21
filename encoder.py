...
class ResCNN(nn.Module):
    def __init__(self, feature_dim=128):
        ...
        self.conv_end = nn.Sequential(
            nn.Conv3d(
                in_channels=64, out_channels=feature_dim, kernel_size=3, padding=3 // 2
            ),
        )
...
def res_block(nf):
    return ResSequential(
        [conv(nf, nf, actn=True), conv(nf, nf)], 1.0
    )  # this is best one
...
class SRResnet(nn.Module):
    def __init__(self, nf=64, feature_dim=128):
        ...
        features = [conv(1, nf)]
        for i in range(18):
            features.append(res_block(nf))
        features += [conv(nf, nf), conv(nf, feature_dim)]
